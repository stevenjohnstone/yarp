...
build/fuzz.parse: $(SOURCES) fuzz/parse.c fuzz/fuzz.c
	$(Q) $(MAKEDIRS) $(@D)
	...
	$(Q) AFL_USE_UBSAN=1 AFL_UBSAN_VERBOSE=1 afl-clang-lto $(DEBUG_FLAGS) $(CPPFLAGS) $(CFLAGS) $(FUZZ_FLAGS) -DIGNORE_ASSERTS -fsanitize=fuzzer,undefined -ggdb3 -std=c99 -Iinclude -o $@.ubsan $^
...
fuzz-run: fuzz-docker-build fuzz-gen-templates
	$(ECHO) "running fuzzer"
	$(Q) docker run --rm -v $(CURDIR):/prism prism/fuzz /bin/bash -c "FUZZ_FLAGS=\"$(FUZZ_FLAGS)\" make build/fuzz.parse"
...
.PHONY: clean fuzz-clean fuzz-docker-build fuzz-gen-templates
